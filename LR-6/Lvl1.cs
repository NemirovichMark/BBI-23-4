...
{
    for (int i = 0; i < count; i++) if (names[i] == name)
            return i;
    return -1;
}

Person[] people = new Person[NamesSorted.Length];
for (int i = 0; i < NamesSorted.Length; i++) people[i] = new Person(NamesSorted[i], NamesCalls[i], count); // преобразую полученные два массива в структуру
