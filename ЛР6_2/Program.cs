...
namespace ЛР6_2
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            players[2] = new Sportsmen("Андреев", 9, 8, 6);
            double[] end = new double[3];
            for (int i = 0; i < 3; i++)
            {
                PoiskBest(players[i].Getrez1, players[i].Getrez2, players[i].Getrez3, ref end[i]);
            }
            for (int i = 0; i < 3; i++)
            {
        ...
        static void PoiskBest(double a, double b, double c, ref double maxim)
        {
            double maxx = 0;
