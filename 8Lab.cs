...
public class Task3 : Task
{
    public override string ToString(string input)
    {
        ...
        {
            if (i + maxLength < input.Length)
            {
                int lastIndex = input.LastIndexOf(' ', Math.Min(i + maxLength, input.Length));
                result += input.Substring(i, lastIndex - i).Trim() + Environment.NewLine;
                i = lastIndex;
            }
            else
            {
                result += input.Substring(i).Trim() + Environment.NewLine;
            }
        }
        return result;
    }
...
public class Task5 : Task
{
    public override string ToString(string input)
    {
        ...
        int totalWords = Regex.Matches(input, @"\b\w+\b").Count;
        ...
        var sortedLettersCount = lettersCount.OrderByDescending(x => x.Value);

        string result = "Задача 5:\n";
        foreach (var kvp in sortedLettersCount)
        {
